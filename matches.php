...
try {
    ...
    switch ($action) {
        case 'save_match':
            ...
            $player2Name    = $input['player2_name'] ?? ($gameMode === 'pve' ? 'Computer (AI)' : 'Player 2');
            $boardStateJson = !empty($input['board_state_json']) ? json_encode($input['board_state_json']) : null;
            $moveHistoryJson = !empty($input['move_history_json']) ? json_encode($input['move_history_json']) : null;
            ...
            if ($player1Id && in_array($result, ['p1_won', 'p2_won', 'draw'])) {
                ...
                // Fetch fresh stats
                $userStmt = $db->prepare("SELECT id, username, email, rating, wins, losses, draws, total_chopped FROM users WHERE id = ?");
        ...
        case 'get_history':
            ...
            $stmt = $db->prepare("
                SELECT id, player1_name, player2_name, game_mode, ai_difficulty,
                       ...
                       moves_count, p1_chopped, p2_chopped, created_at
                FROM matches
                WHERE player1_id = ?
