...
// TEST 1: 50-Square Engine - Capturing over crown row does NOT crown (promoted = false)
console.log('Test 1: 50-Square Engine Capturing Over Crown Row (Not a Crown)');
{
  const board = new DraughtsBoard50({ ruleMode: 'nigeria' });
  board.board.fill(EMPTY);
  // P1 man at 13. Enemy at 8. Reaching 4 (king row for P1).
  // Enemy 2 at 9 (adjacent to 4). Landing at 15 (off king row).
  ...
  assert(cap.promoted === false, `Move marked as promoted=false (capturing over crown row is NOT a crown)`);
  assert(cap.jumpedSquares.length === 2 && cap.jumpedSquares[0] === 8 && cap.jumpedSquares[1] === 9, `Jumped both enemies 8 and 9`);
}

// TEST 2: 10x10 UI Engine - Seed capturing over crown row remains a seed (does NOT crown)
console.log('\nTest 2: 10x10 Engine Interactive Capturing Over Crown Row (Remains a Seed)');
{
  ...
  assert(res1.justPromoted === false, `justPromoted is FALSE on step 1 (seed not crowned mid-jump)`);
  assert(engine.board[0][2].isKing === false, `Piece at (0,2) is still a SEED`);
  assert(engine.activeMultiJump !== null, `activeMultiJump is active`);

  // Step 2: Continuation is a SEED jump to (2,4)
  const legals2 = engine.getAllLegalMoves(PLAYER_1);
  assert(legals2.length === 1, `Piece has 1 continuation jump to (2,4)`);
  const res2 = engine.makeMove(legals2[0]);
  assert(res2.turnEnded === true, `Turn ends after second capture`);
  assert(res2.justPromoted === false, `justPromoted is FALSE upon turn completion (not a crown when capturing over crown)`);
  assert(engine.board[2][4].isKing === false, `Piece at final destination (2,4) remains a SEED (NOT a crown)`);
  assert(engine.board[1][1] === null && engine.board[1][3] === null, `Both jumped pieces removed from board`);
}

// TEST 3: All AI Difficulty Levels (1 to 6) - Capturing over crown does NOT crown across all AI levels
console.log('\nTest 3: AI Difficulty Levels (1 to 6) Overcrown Execution (Check Grandmaster & All Levels)');
{
  ...
  for (const lvl of levels) {
    ...
    // Enemy 2 at (8,4) (adjacent to 9,3). Landing at (7,5) (off crown row).
    ...
    assert(res1.justPromoted === false, `Level ${lvl.num} justPromoted is FALSE on step 1`);
    assert(engine.board[9][3].isKing === false, `Level ${lvl.num} Piece at (9,3) remains a SEED`);

    // Step 2: AI executes continuation capture as a seed, landing on (7,5)
    ...
    assert(res2.justPromoted === false, `Level ${lvl.num} justPromoted is FALSE (capturing over crown is not a crown)`);
    assert(engine.board[7][5].isKing === false, `Level ${lvl.num} Piece remains a SEED at (7,5)`);
  }
}

// TEST 4: Seed stopping ON the crown row DOES crown into King
console.log('\nTest 4: Seed Stopping ON Crown Row Crowns into King');
{
  const engine = new NigerianDraughtsEngine({ boardSize: 10, ruleMode: 'nigeria' });
  for (let r = 0; r < 10; r++) for (let c = 0; c < 10; c++) engine.board[r][c] = null;
  // P1 man at (2,0). Enemy 1 at (1,1) -> lands at (0,2) (crown row). No further enemies.
  engine.board[2][0] = { id: 1, player: PLAYER_1, isKing: false };
  engine.board[1][1] = { id: 2, player: PLAYER_2, isKing: false };
  engine.currentTurn = PLAYER_1;
  ...
  assert(res1.turnEnded === true, `Turn ends at (0,2)`);
  assert(res1.justPromoted === true, `justPromoted is TRUE because piece stopped on king row`);
  assert(engine.board[0][2].isKing === true, `Piece at (0,2) is crowned as King!`);
}

// TEST 5: Ghanaian Damii Rule Preservation
console.log('\nTest 5: Ghanaian Damii Rule Preservation (Ends Turn Immediately on Backline)');
{
...
// TEST 6: International FMJD Rule Preservation (Rule 3.5 Man Non-Promotion Mid-Chain)
console.log('\nTest 6: International FMJD Rule Preservation (FMJD Rule 3.5)');
{
