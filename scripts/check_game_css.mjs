...
async function checkGameCSS() {
  const port = 9340;
  const userDataDir = `C:\\Users\\ayoad\\.gemini\\antigravity-ide\\brain\\986c3d57-38e8-4d61-9afb-d6a8faf30da7\\scratch\\edge_css_${Date.now()}`;
      ...
      params: {
        expression: `(() => {
          ...
          return JSON.stringify({
            boardRect: rect,
            frameRect: frameRect,
            boardVisibility: computed.visibility,
            boardOpacity: computed.opacity,
            boardClip: computed.clipPath,
            boardZIndex: computed.zIndex,
            boardPosition: computed.position
          }, null, 2);
        })()`
      }
    }));
  ...
  console.log("Game CSS Info:", evalRes);
  ws.close();
  browserProc.kill();
}

checkGameCSS().catch(console.error);
