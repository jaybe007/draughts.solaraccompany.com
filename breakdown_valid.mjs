...
console.log("Total valid puzzles:", validPuzzles.length);
const byTier = {};
const byRuleset = {};
const byTheme = {};
validPuzzles.forEach(p => {
  const t = p.difficulty?.tier || p.tier || 'unknown';
  byTier[t] = (byTier[t] || 0) + 1;
  const r = p.ruleset || 'nigeria';
  byRuleset[r] = (byRuleset[r] || 0) + 1;
  const th = p.themeName || p.category || 'unknown';
  byTheme[th] = (byTheme[th] || 0) + 1;
});
console.log("By Tier:", byTier);
console.log("By Ruleset:", byRuleset);
console.log("By Theme count:", Object.keys(byTheme).length);
